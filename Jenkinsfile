    pipeline {
        ...
        stages {
            ...
            stage('Create Service YAML') {
                steps {
                    script {
                        ...
                        serviceTemplate = serviceTemplate.replace('${CONTAINER_PORT}', env.CONTAINER_PORT)
            ...
            stage('Clean Up') {
                steps {
                    script {
                        // Hapus file yang dibuat selama proses build
                        deleteDir() // Hapus seluruh direktori kerja
                    }
                }
            }
        }
        ...
        post {
            always {
                // Pastikan workspace dibersihkan meskipun build gagal atau sukses
                cleanWs() // Membersihkan seluruh workspace
            }
        }
    }
